...
flags.DEFINE_string("models_file", "", "JSON file containing models.")
flags.DEFINE_string(
    "remainders_file",
    "",
...
flags.DEFINE_string(
    "model_id_regex",
    "^",
    "Regex of models to train. Model IDs are MD5 hashes" " which match ([a-f0-9]{32}).",
)
flags.DEFINE_string("output_dir", "", "Base output directory.")
flags.DEFINE_integer("worker_id", 0, "Worker ID within this flock, starting at 0.")
flags.DEFINE_integer("worker_id_offset", 0, "Worker ID offset added.")
flags.DEFINE_integer("total_workers", 1, "Total number of workers, across all flocks.")
FLAGS = flags.FLAGS
...
class Evaluator(object):
    ...
    def __init__(
        self, models_file, output_dir, worker_id=0, total_workers=1, model_id_regex="^"
    ):
    ...
    """
        ...
        model_dir = os.path.join(
            self.output_dir, model_id[:2], model_id, "repeat_%d" % model_repeat
        )
        try:
