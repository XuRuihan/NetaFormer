...
def train(config):
    ...
    for epoch_idx in range(start_epoch_idx, config.max_epoch):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            if config.lambda_consistency > 0:
                ...
                loss_con = (
                    lossConsistency(source_pre, auged_pre) * config.lambda_consistency
                )
        ...
        if (epoch_idx + 1) % test_freq == 0:
            ...
            LOG.log(
                ...
                "ErrBnd(0.1) {:.5f}, Best_ErrB {:.5f}, EMA_ErrBnd(0.1) {:.5f}, Best_EMA_ErrB {:.5f}, ".format(
                    tau,
                    best_tau,
                    tau_ema,
                    best_tau_ema,
    ...
    LOG.log(
        "Training Finished! Best MAPE: %11.8f, Best ErrBnd(0.1): %11.8f; Best MAPE on EMA: %11.8f, Best ErrBond(0.1) on EMA: %11.8f"
...
def test(config):
    ...
    LOG.log(
        "Test Finished! KT: %11.8f, MAPE: %11.8f, ErrBnd(0.1): %11.8f" % (tau, acc, err)
    )
